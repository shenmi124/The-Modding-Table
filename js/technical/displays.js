function prestigeButtonText(layer) {
	...
	if (tmp[layer].type == "normal")
		return `${player[layer].points.lt(1e3) ? (tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : ((options.ch || modInfo.otherLanguageMod==false) ? "重置以获得 " : 'Reset for ' )) : ""}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource} ${tmp[layer].resetGain.lt(100) && player[layer].points.lt(1e3) ? `<br><br>`+((options.ch || modInfo.otherLanguageMod==false) ? "下一个在:" : "Neat at:")+` ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAt) : format(tmp[layer].nextAt))} ${tmp[layer].baseResource}` : ""}`
	if (tmp[layer].type == "static")
		return `${tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : ((options.ch || modInfo.otherLanguageMod==false) ? "重置以获得 " : 'Reset for ' )}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource}<br><br>${player[layer].points.lt(30) ? (tmp[layer].baseAmount.gte(tmp[layer].nextAt) && (tmp[layer].canBuyMax !== undefined) && tmp[layer].canBuyMax ? ((options.ch || modInfo.otherLanguageMod==false) ? "下一个:" : "Next:") : ((options.ch || modInfo.otherLanguageMod==false) ? "需求:" : "Req:")) : ""} ${formatWhole(tmp[layer].baseAmount)} / ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAtDisp) : format(tmp[layer].nextAtDisp))} ${tmp[layer].baseResource}		
		`
	if (tmp[layer].type == "none")
		return ""
    
        return (options.ch || modInfo.otherLanguageMod==false) ? "你需要为声望按钮写一段文字" : 'You need prestige button text'
}
