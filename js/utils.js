...
function updateMilestones(layer) {
	...
	for (id in layers[layer].milestones) {
		if (!(hasMilestone(layer, id)) && layers[layer].milestones[id].done()) {
			...
			if (tmp[layer].milestonePopups || tmp[layer].milestonePopups === undefined) doPopup("milestone", geti18n()?tmp[layer].milestones[id].requirementDescription : tmp[layer].milestones[id].requirementDescriptionI18N, geti18n()?"获得里程碑!" : "Milestone Gotten!", 3, tmp[layer].color);
...
function updateAchievements(layer) {
	...
	for (id in layers[layer].achievements) {
		if (isPlainObject(layers[layer].achievements[id]) && !(hasAchievement(layer, id)) && layers[layer].achievements[id].done()) {
			...
			if (tmp[layer].achievementPopups || tmp[layer].achievementPopups === undefined) doPopup("achievement", geti18n()?tmp[layer].achievements[id].name : tmp[layer].achievements[id].nameI18N, geti18n()?"获得成就!" : "Achievement Gotten!", 3, tmp[layer].color);
		}
	}
}
...
function addTime(diff, layer) {
	...
	if (time + 0 !== time) {
		console.log(geti18n()?"检测到内存缺失. 尝试修复中..." : "Memory leak detected. Trying to fix...")
		time = toNumber(time)
		if (isNaN(time) || time == 0) {
			console.log(geti18n()?"修复失败! 重置中..." : "Couldn't fix! Resetting...")
